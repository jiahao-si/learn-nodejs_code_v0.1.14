...
OutgoingMessage.prototype.sendHeaderLines = function (first_line, headers) {
  ...
  if (
    sent_content_length_header == false &&
    sent_transfer_encoding_header == false
  ) {
    if (this.use_chunked_encoding_by_default) {
      /**
       * Transfer-Encoding: chunked 的特殊处理
       * 体现在 chunked_encoding 的不同处理上，如 send body ，行间会加一些换行 CRLF = '\r\n'
       */
  ...
  while (queue[0]) {
    ...
    while (message.output.length > 0) {
      if (
        ...
        connection.readyState !== 'writeOnly'
      ) {
        return false;
      }
...
function connectionListener(connection) {
  ...
  createIncomingMessageStream(
    connection,
    function (incoming, should_keep_alive) {
      var req = incoming;
...
/**
 * 这是用作 http.createClient
 * 作为 client 客户端去请求别的服务
 * @param {*} port
 * @param {*} host
 * @returns
 */
